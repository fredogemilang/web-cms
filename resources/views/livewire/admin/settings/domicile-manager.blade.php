    ...
    <div class="rounded-3xl bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#272B30] overflow-hidden shadow-sm">
        ...
        <div class="px-8 py-6 border-t border-gray-100 dark:border-[#272B30] flex items-center justify-between">
            <p class="text-sm font-medium text-[#6F767E]">
                Showing {{ $domiciles->firstItem() }} to {{ $domiciles->lastItem() }} of {{ $domiciles->total() }} domiciles
            </p>
            <div class="flex items-center gap-2">
                @if($domiciles->onFirstPage())
                <button disabled
                ...
                @foreach($domiciles->getUrlRange(max(1, $domiciles->currentPage() - 2), min($domiciles->lastPage(), $domiciles->currentPage() + 2)) as $page => $url)
                    @if($page == $domiciles->currentPage())
